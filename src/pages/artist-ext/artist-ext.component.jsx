...
const ArtistExt = (props) => {
	...
	const artist = artists.find(
		(el) => el.name === `${props.match.params.artistId}`
	)
	console.log(artist)
	return (
		<PageContainer>
			<ArtistCont>
				{artist ? (
					<>
						<Banner
							style={{ backgroundImage: `url(${urlFor(artist.banner).url()})` }}
						>
						...
						<ReleaseCont>
							...
							<ReleaseItem
								src={`https://open.spotify.com/embed/artist/${artist.releases[0].spotifyURI}`}
								frameborder='0'
							...
							{artist.videos ? (
								<VideoCont>
									...
									{artist.videos.map((item, id) => (
										<VideoItem key={id} src={item.youtubeLink}></VideoItem>
									))}
								</VideoCont>
							) : null}
							{artist.articles ? (
								<ArticleCont>
									...
									{artist.articles.map((item, id) => (
										<Article key={id} href={item.link}>
